...
var context = cubism.context()
    //.step(7*100)
//.step(5 * 60 * 1000) // seven minutes per value
//.step(1 * 1000) // one sec per value
.step(24*60*60*1000) // one year per value
///.step(365*24*60*60*1000) // one year per value
    .size(960)
    .stop();
...
function transition() {
 ...
 // doToggle(toggleState);//CF
  ...
  for (var i in graph) {
    ...
    //CF
    horizonData = graph[i].data;
    updateHorizon();

  if (streaming) {
    for (var i in graph) {
      if (graph[i].active) {
          //horizonData = graph[i].streamData;
          //updateHorizon();
   ...
   // horizonData = streaming ? graph[i].streamData : graph[i].data;
    //updateHorizon();
 ...
 //   console.log(i,graph[i].streamData ,'-', graph[i].data); //CF
...
function createOrUpdateGraph(i) {
  ...
  //console.log(i,graph[i].streamData ,'-', graph[i].data); //CF
   // horizonData = graph[0].streamData; //CF
...
function updateHorizon() {
    console.log(horizonData.length);
    
d3.select("body").selectAll(".horizon")
    //.data(d3.range(0, horizonData.length).map(random))
.data(d3.range(0, horizonData.length).map(stock))
  .enter().insert("div", ".bottom")
    .attr("class", "horizon")
    .call(context.horizon().extent([-0.5, 0.5]));
...
function stock(name) {
  ...
  return context.metric(function(start, stop, step, callback) {
      //console.log('horizonData[name]',horizonData[name]);
      var values = [],
          lookup = {},
          i = start.getTime();
      horizonData[name].forEach(function(d){
          
          //d.Date = new Date(d.Date);
          
          d.x = formatYear(d.x);
          //console.log(name,d,d.x);
          lookup[d.x] = d;
          
          //console.log(name,d,'y',d.y ,'y0', d.y0, ' ',d.y - d.y0);
          //values.push(d.y - d.y0);
          //console.log(name,'y',d.y);
          
          
          //values.push( streaming? (d.y + d.y0) : d.y);
          //values.push( d.y);
      });
      
      while ((i += step) < stop) {
          
        var key = formatYear(new Date(i));
        var value = key in lookup ? lookup[key].y: null;
        console.log(i,step,stop,key,value);
        values.push(value);
      }
